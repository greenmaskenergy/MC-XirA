...
bitsadmin.exe /transfer "" /priority FOREGROUND https://onehit.eu/MC_ModLists/XirA/mods %cd%/mods
...
if not exist %cd%\%%A (
    echo missing: %%A
    bitsadmin.exe /transfer "Mod (!progress! / !ModCount!):  %%A"  /priority FOREGROUND https://github.com/greenmaskenergy/MC-XirA/raw/master/%%A %cd%\%%A
) else (
    cls
